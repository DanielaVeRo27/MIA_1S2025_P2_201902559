...
// conversion K o M a bytes
func Conversion(size int, unit string) (int, error) {
	// Convertir el tamaño a bytes según la unidad especificada
	switch unit {
	case "K":
	...
	default:
		return 0, errors.New("UNIDAD INVALIDA") // error
	}
}

// Permite identificar diferentes discos con letras, como en Windows (C:, D:, E:).
// Para identificar de manera única cada disco montado
// Lista con todo el abecedario para identificar particiones
var alphabet = []string{
...
// path -> contador de particiones
var pathToPartitionCount = make(map[string]int)

// Índice de que letra asignar a continuación
var nextLetterIndex = 0

// Obtenie la letra asignada a un path y el siguiente índice de partición
func GetLetterAndPartitionCorrelative(path string) (string, int, error) {

	// Asignar  al  letra al path
